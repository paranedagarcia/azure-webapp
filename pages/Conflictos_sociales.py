...
with tabPanel:
    ...
    df_conflict_type = df.groupby(
        ['YearMonth', 'Tipo de conflicto social']).size().reset_index(name='count')
    df_conflict_type['YearMonth'] = df_conflict_type['YearMonth'].astype(str)

    with st.expander("Tipos de conflictos"):
        conflictos = sorted(
            df["Tipo de conflicto social"].dropna().unique().tolist())
    ...
    fig_conflict_type.update_layout(
        xaxis_title='Fecha', yaxis_title='Número de conflictos')

    st.plotly_chart(fig_conflict_type, use_container_width=True)
